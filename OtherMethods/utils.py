...
def sample_baylr(
        ...
        sigsq=None, a_prio=None, b_prio=None, sample_shape=()):
    import tensorflow as tf
    import tensorflow_probability as tfp
    tfd = tfp.distributions

    if (a_prio is not None) and (b_prio is not None):
        sample_sigsq = True
    elif sigsq is not None:
        sample_sigsq = False
    else:
        raise ValueError('sigsq and (a,b) cannot be both None')

    if sample_sigsq:
        beta_postmea, beta_postcov, a_post, b_post = fit_baylr(
                y, x, beta_priomea, beta_priocov, a_prio, b_prio)
        sigsq_postdist = tfd.InverseGamma(a_post, b_post)
        sigsq = sigsq_postdist.sample().numpy()
    else:
        beta_postmea, beta_postcov = fit_baylr(
                y, x, beta_priomea, beta_priocov)
    beta_postdist = tfd.MultivariateNormalTriL(
            np.swapaxes(beta_postmea, -1, -2),
            tf.linalg.cholesky(
                beta_postcov * sigsq[..., np.newaxis, np.newaxis]))
    beta_postsamp = np.swapaxes(
            beta_postdist.sample(sample_shape).numpy(), -1, -2)
    return beta_postsamp
